...
it('1 can Create a Star', async () => {
  ...
  await instance.createStar('Awesome Star!', tokenId, "AS", { from: accounts[0] })
  const response = await instance.tokenIdToStarInfo.call(tokenId);
  assert.equal(response.name, 'Awesome Star!');
  assert.equal(response.symbol, "AS");
});
...
it('2 lets user1 put up their star for sale', async () => {
  ...
  await instance.createStar('awesome star', starId, "AS", { from: user1 });
...
it('6 can add the star name and star symbol properly', async () => {
  ...
  let tokenId = 6;
  let instance = await StarNotary.deployed();
  await instance.createStar('Another Awesome Star!', tokenId, "AAS", { from: accounts[0] })
  // 2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
  const response = await instance.tokenIdToStarInfo.call(tokenId);
  assert.equal(response.name, 'Another Awesome Star!');
  assert.equal(response.symbol, "AAS");
});
...
it('8 lets a user transfer a star', async () => {
  ...
  let tokenId = 8;
  let instance = await StarNotary.deployed();
  await instance.createStar('Another Star!', tokenId, "AS", { from: accounts[0] });
...
it('9 lookUptokenIdToStarInfo test', async () => {
  ...
  let tokenId = 9;
  let instance = await StarNotary.deployed();
  await instance.createStar('Dummy Star!', tokenId, "DS", { from: accounts[0] });
